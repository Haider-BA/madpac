...
#ifndef PEANO_APPLICATIONS_LATTICEBOLTZMANN_BLOCKLATTICEBOLTZMANN_MAPPINGS_SpacetreeGrid2BlockVTKOutput_H_
#define PEANO_APPLICATIONS_LATTICEBOLTZMANN_BLOCKLATTICEBOLTZMANN_MAPPINGS_SpacetreeGrid2BlockVTKOutput_H_
...
namespace peano {
  namespace applications {
    namespace latticeboltzmann {
      ...
      namespace mappings {
        class SpacetreeGrid2BlockVTKOutput;
      }
}
}
}
 ...
 */
class peano::applications::latticeboltzmann::blocklatticeboltzmann::mappings::SpacetreeGrid2BlockVTKOutput {
  private:
    /**
     * Logging device.
     */
  ...
  public:
     ...
     * modification.
     */
    SpacetreeGrid2BlockVTKOutput();

    /**
     * Destructor. Typically does not implement any operation.
     */
    virtual ~SpacetreeGrid2BlockVTKOutput();
 ...
 \endcode
     ...
     *
     * @see SpacetreeGrid2BlockVTKOutput()
     */
    void beginIteration(
  ...
  private:
    ...
    /** for information transfer in overlap region */
    const peano::applications::latticeboltzmann::blocklatticeboltzmann::BlockRestrictionProlongation _blockRestrictionProlongation;

    /** simulation data on all grid levels */
    ...
    tarch::plotter::griddata::Writer::CellDataWriter*                             _densityWriter;
    tarch::plotter::griddata::Writer::CellDataWriter*                             _velocityWriter;
    tarch::plotter::griddata::Writer::CellDataWriter*                             _collisionModelWriter;
    tarch::plotter::griddata::Writer::CellDataWriter*                             _vertexNumberWriter;

    /** does the plotting. The coarseVertex is needed as - in case of overlap cells or newly created sub vertices,
     *  we do not have valid data in the fine grid vertex and thus need to plot the cell of the coarse grid vertex.
     *  subBlock specifies the "position" of the fine grid vertex in the coarse vertex grid. Each component
     *  has a number between 0 and 2. See BlockRestrictionProlongation for details on that. position, h refer to the
     *  fine (Peano) grid.
     */
    void plotVertex(
    ...
    /** computes the current grid level from fineGridH, _referenceMeshsize and _referenceLevel */
    int getLevel(double fineGridH) const {
      return _referenceLevel - (int)(log(fineGridH/_referenceMeshsize)/log(3.0));
    }

...
#ifndef PEANO_APPLICATIONS_POISSON_JACOBITUTORIAL_MAPPINGS_RegularGrid2JacobiStep_H_
#define PEANO_APPLICATIONS_POISSON_JACOBITUTORIAL_MAPPINGS_RegularGrid2JacobiStep_H_
...
class peano::applications::poisson::jacobitutorial::mappings::RegularGrid2JacobiStep {
  private:
    /**
     * Logging device.
     */
    ...
    /**
     * The relaxation factor @f$ \omega @f$ is held by the state. As a mapping
     * may not have persistent data in-between a finishedIteration() and
     * startIteration() call, we reset this value each time startIteration() is
     * invoked.
     */

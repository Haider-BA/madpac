...
tarch::logging::Log peano::kernel::datatraversal::dForRange::_log( "peano::kernel::datatraversal::dForRange" );


peano::kernel::datatraversal::dForRange::dForRange(const dForRange& range):
...
peano::kernel::datatraversal::dForRange::dForRange( const tarch::la::Vector<DIMENSIONS,int>&  range, int grainSize, bool handleBoundaryOfDomainSequential ):
  _offset(0),
  _range(range),
...
bool peano::kernel::datatraversal::dForRange::isSequentialBoundaryRange() const {
  return _handleBoundaryOfDomainSequential;
}


peano::kernel::datatraversal::dForRange::dForRange( dForRange& range, Split ):
  _offset(range._offset),
  _range(range._range),
  _grainSize( range._grainSize),
  _handleBoundaryOfDomainSequential( range._handleBoundaryOfDomainSequential ),
...
bool peano::kernel::datatraversal::dForRange::empty() const {
  logTraceIn( "empty()" );
...
bool peano::kernel::datatraversal::dForRange::is_divisible() const {
...
std::string peano::kernel::datatraversal::dForRange::toString() const {
...
tarch::la::Vector<DIMENSIONS,int> peano::kernel::datatraversal::dForRange::getOffset() const {
  return _offset;
}


tarch::la::Vector<DIMENSIONS,int> peano::kernel::datatraversal::dForRange::getRange() const {
  return _range;
}

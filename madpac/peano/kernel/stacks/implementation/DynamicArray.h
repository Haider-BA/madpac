...
#ifndef _PEANO_KERNEL_STACKS_IMPLEMENTATION_DYNAMICARRAY_H_
#define _PEANO_KERNEL_STACKS_IMPLEMENTATION_DYNAMICARRAY_H_
 ...
 */
template<typename T, bool copyconstruct, bool shrink>
class peano::kernel::stacks::implementation::DynamicArray {
...
public:
	...
	void push_back(const T& data) {
		if (_finish == _endOfStorage) {
			...
			size_t newCapacity = (size_t) ((capacity() * _capacityIncrement) > capacity()+1 ? (capacity() * _capacityIncrement) : capacity()+1);
	...
	void pop_back() {
		...
		if (shrink && size() < capacity() / _capacityIncrement) {
			reallocate((size_t)(capacity() / _capacityIncrement));
		}
	}
	...
	iterator& erase(iterator& position) {
		if (_finish > _start) {
			...
			*position._current = *_finish;
			...
			assertion(_finish == _start);
			assertion(_finish == position._current);
		}
		_finish->~T();

...
int main(int argc, char** argv) {
	...
	global_log = new Log::Logger(Log::MARDYN_Info);
#ifdef ENABLE_MPI
	...
	if (options.is_set("verbose") && options.get("verbose"))
		global_log->set_log_level(Log::MARDYN_All);
	...
	if (tests) {
		...
		Log::logLevel testLogLevel = options.is_set("verbose") && options.get("verbose") ? Log::MARDYN_All : Log::MARDYN_Info;

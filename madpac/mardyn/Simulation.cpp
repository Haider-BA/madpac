...
void Simulation::simulate() {
	simulateStart();
	for (_simstep = _initSimulation; _simstep <= _numberOfTimesteps; ) {
...
void  Simulation::simulateStart() {
	...
	uCAT = _pressureGradient->getUCAT();
	...
	_ensemble = new CanonicalEnsemble(_moleculeContainer, &(_domain->getComponents()));
	_ensemble->updateGlobalVariable(NUM_PARTICLES);
	global_log->debug() << "Number of particles in the Ensemble: " << _ensemble->N() << endl;
	_ensemble->updateGlobalVariable(ENERGY);
	global_log->debug() << "Kinetic energy in the Ensemble: " << _ensemble->E() << endl;
	_ensemble->updateGlobalVariable(TEMPERATURE);
	global_log->debug() << "Temperature of the Ensemble: " << _ensemble->T() << endl;
...
void Simulation::prepareTimestep() {
  _loopTimer->start();
...
void Simulation::forceCalculationEtc() {
  // Force calculation
  ...
  // Inform the integrator about the calculated forces
          ...
          if (!_domain->NVE()) {
                  ...
                  if (_domain->severalThermostats()) {
                          ...
                          std::cout << _domain->getGlobalBetaTrans() << std::endl;
                          std::cout << _domain->getGlobalBetaRot() <<std::endl;
                  }
          }
}


void Simulation::finishTimestep() {
  _moleculeContainer->calcVelocity();
...
void Simulation::simulateFinish() {
	_ioTimer->start();
...
void Simulation::output(unsigned long simstep) {
	...
	global_log->info() << "Simstep = " << simstep << "\tT = " << _domain->getGlobalCurrentTemperature() << "\tU_pot = "
	        << _domain->getAverageGlobalUpot() << "\tp = " << _domain->getGlobalPressure()

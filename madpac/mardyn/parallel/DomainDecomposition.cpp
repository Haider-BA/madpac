...
#ifdef MADPAC_X
				if(madpac::PeanoCouplingService::getInstance().bEnabled() && d < madpac::PeanoCouplingService::getInstance().getDim()) {//todo:remove
					if (_coords[i] == 0)
						regToSendLow[i] = rmin[i];
				}
#endif
				regToSendHigh[i] = rmax[i] + halo_L[i];
#ifdef MADPAC_X
				if(madpac::PeanoCouplingService::getInstance().bEnabled() && d < madpac::PeanoCouplingService::getInstance().getDim()) { //todo:remove
					if (_coords[i] == _gridSize[i] - 1)
						regToSendHigh[i] = rmax[i];
				}
#endif
			}
			switch (direction) {
...
#ifdef MADPAC_LBMD_COUPLING
			if((madpac::PeanoCouplingService::getInstance().bEnabled() && _coords[d] == 0 && direction == LOWER && d < madpac::PeanoCouplingService::getInstance().getDim())
			|| (madpac::PeanoCouplingService::getInstance().bEnabled() && _coords[d] == _gridSize[d] - 1 && direction == HIGHER  && d < madpac::PeanoCouplingService::getInstance().getDim())) {
			//	std::cout << getRank() << " "<< numsend << std::endl;
				if(madpac::PeanoCouplingService::getInstance().isInit() && madpac::PeanoCouplingService::getInstance().getConfig()->getMovingWall())
					madpac::PeanoCouplingService::getInstance().getDataContainer()->addNumInsertMolecules(numsend);
				//if(numsend != 0)
					//std::cout << getRank() << " XY "<< numsend << std::endl;
			}
#endif
...
#ifdef MADPAC_LBMD_COUPLING
			if((madpac::PeanoCouplingService::getInstance().bEnabled() && _coords[d] == 0 && direction == HIGHER && d < madpac::PeanoCouplingService::getInstance().getDim())
			|| (madpac::PeanoCouplingService::getInstance().bEnabled() && _coords[d] == _gridSize[d] - 1 && direction == LOWER  && d < madpac::PeanoCouplingService::getInstance().getDim())) {
				//std::cout << getRank() << " "<< numrecv << std::endl;
				if(madpac::PeanoCouplingService::getInstance().isInit() && !madpac::PeanoCouplingService::getInstance().getConfig()->getMovingWall()) {
					madpac::PeanoCouplingService::getInstance().getDataContainer()->addNumInsertMolecules(numrecv);
				}

				//std::cout << getRank() << " XY "<< numrecv << std::endl;
			} else
#endif

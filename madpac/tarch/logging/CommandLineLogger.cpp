...
void tarch::logging::CommandLineLogger::setLogFormat(const tarch::logging::configurations::LogOutputFormatConfiguration& configuration) {
  _logColumnSeparator        = configuration.getLogColumnSeparator();
  _logTimeStamp              = configuration.getLogTimeStamp();
  _logTimeStampHumanReadable = configuration.getLogTimeStampHumanReadable();
  _logMachineName            = configuration.getLogMachineName();
  _logMessageType            = configuration.getLogMessageType();
  _logTrace                  = configuration.getLogTrace();
}
...
bool tarch::logging::CommandLineLogger::filterOut(
  ...
  for (FilterList::const_iterator p = _filterlist.begin(); p!=_filterlist.end(); p++ ) {
    int length = p->_namespaceName.size();

...
namespace io
{
     ...
     */
		...
		double convertValueStringToDouble(const std::string& value) const {
		  ...
		  if ( value.find("/") == std::string::npos ) {  //no "/" in
		    if (value.find("pi") == std::string::npos) { //no "pi" in
		      ...
		      std::string left  = value.substr( 0, value.find("pi") );
		      std::string right = value.substr( value.find("pi")+2, value.size()-value.find("pi")-1 );
		      double leftValue;
		    ...
		    std::string right = value.substr( value.find("/")+1, value.size()-value.find("/")-1 );
      ...
      std::string valueString(attributeValue);
	    for (int i = 0; i < num; i++){
	      ...
	      vec(i) = convertValueStringToDouble(tmp1);
	    }
	    return vec;
	  }

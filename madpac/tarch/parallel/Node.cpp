...
tarch::logging::Log tarch::parallel::Node::_log("parallel::Node");
...
void tarch::parallel::Node::plotMessagesInQueueToError(int rank) {
  MPI_Status   status;
  int          flag;

  MPI_Iprobe(rank, MPI_ANY_TAG, _communicator, &flag, &status);

  if (flag==0) {
    std::ostringstream msg;
    msg << "no message in queue from rank " << rank;
    _log.error( "plotMessagesInQueueToError(int)", msg.str() );
  }
  else {
    std::ostringstream msg;
    msg << "there are messages in queue from rank " << rank << ". Message tags=";
    bool writeComma = false;
    for (int tag=0; tag<4; tag++) {
      MPI_Iprobe(rank, tag, _communicator, &flag, &status);
      if (flag) {
        if (writeComma) {
          msg << ",";
        }
...
void tarch::parallel::Node::triggerDeadlockTimeOut(
  ...
  out << "operation " << className << "::" << methodName << " on node "
      ...
      << "than " << _deadlockTimeOut << " seconds "
      << "for a message from node " << communicationPartnerRank << ". Timeout";
  _log.error( "triggerDeadlockTimeOut(...)", out.str() );

  if ( comment!="" ) {
	_log.error( "triggerDeadlockTimeOut(...)", comment );
  }
...
bool tarch::parallel::Node::isTimeOutDeadlockEnabled() const {
  return !isMasterProcess() && _deadlockTimeOut > 0;
}
...
int tarch::parallel::Node::getMasterProcessRank() {
  return 0;
}


bool tarch::parallel::Node::isMasterProcess() const {
  assertion(_initIsCalled);
  return getRank() == getMasterProcessRank();
}
...
void tarch::parallel::Node::setCommunicator( int communicator ) {
  assertion( communicator>=0 );
